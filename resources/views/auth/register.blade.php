                ...
                <form class="content" id="register-form" method="POST" action="{{route('register')}}">
                    ...
                    <div class="row">
                        <div class="col col-12 col-xl-12 col-lg-12 col-md-12 col-sm-12">
                            <a href="javascript:;" class="explanation"
                               style="border-bottom: 1px dotted gray; color: gray">
                            ...
                            <div class="form-group label-floating is-select">
                                ...
                                <select id="profile_type_input" name="profile_type" class="selectpicker form-control"
                                        required>
                                    @foreach ($userTypes as $userTypeKey=> $userTypeValue)
                                        <option @if (old('profile_type') == $userTypeValue) selected
                                                @endif value="{{$userTypeValue}}">{{__('auth.register_page.'.$userTypeKey)}}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col col-12 col-xl-12 col-lg-12 col-md-12 col-sm-12">
                            ...
                            <div class="form-group label-floating is-select" id="gender_input_group">
                                ...
                                <select name="gender" class="selectpicker form-control" required>
                                    @foreach ($genders as $genderKey => $genderValue)
                                        <option @if (old('gender') == $genderKey) selected
                                                @endif value="{{$genderKey}}">{{__('auth.register_page.'.$genderValue)}}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="remember">
                                <div class="checkbox">
                                    <label>
                                        ...
                                        {{__('auth.register_page.i_accept')}} <a href="{{route('web.terms-of-use')}}"
                                                                                 style="color: gray; text-decoration: underline;"
                                                                                 target="_blank">{{__('auth.register_page.terms_and_conditions')}}</a> {{__('auth.register_page.of_website')}}
                                    </label>
                            ...
                            <button type="submit" class="btn btn-green btn-lg full-width"
                                    style="color: white;">{{__('auth.register_page.complete_registration')}}</button>
                            <a href="{{route('login')}}" class="btn btn-grey-lighter btn-lg full-width"
                               style="color: white;">{{__('auth.login')}}</a>
                        </div>
                    </div>
                </form>
            </div>
