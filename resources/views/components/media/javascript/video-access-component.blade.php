        ...
        response
            .done(function (data) {
                ...
                $.each(hashtags, function (index, value) {
                    hashtagsHtml += '<a href="{{route('
                    web.search
                    ', ['
                    type
                    ' => SearchConstants::SEARCH_MEDIA])}}&term=' + value.name + '">' + value.hashtag_name + '</a>';
                    hashtagsHtml += ' ';
                });
                ...
                ')}}', 'POST', {id: $(videoElement).data('video-id')}
                )
                response
                    .done(function (data) {
                        $('#open-photo-popup-v2 #views').text(data.views);
